  ...
  <View style={styles.addressBlock}>
    ...
    <View style={styles.row}>
      <TextInput
        ...
        onChangeText={(t) => setFormData({ ...formData, [`${prefix}_zip`]: t.replace(/[^0-9]/g, '') })}
      />
      <TextInput
        style={[styles.input, { flex: 2 }]}
        placeholder="Város *"
        value={formData[`${prefix}_city`]}
        onChangeText={(t) => setFormData({ ...formData, [`${prefix}_city`]: t })}
      />
    </View>
    <TextInput
      ...
      onChangeText={(t) => setFormData({ ...formData, [`${prefix}_address`]: t })}
    />
  </View>
);
...
export default function SendParcelScreen() {
  ...
  const sizeMapping: { [key: string]: { apiValue: string; price: number; courierId: number } } = {
  ...
  const [formData, setFormData] = useState({
    ...
    p_zip: "", p_city: "", p_address: "",
    d_zip: "", d_city: "", d_address: "",
    package_size: "S",
...
const validate = () => {
  ...
  // 1. Kötelező mezők kitöltöttségének ellenőrzése
  if (
    !p_zip.trim() || !p_city.trim() || !p_address.trim() || 
    !d_zip.trim() || !d_city.trim() || !d_address.trim() || 
    !receiver_name.trim() || !receiver_phone.trim()
  ) {
    showAlert("Hiba", "Minden csillaggal (*) jelölt mezőt kötelező kitölteni!");
    return false;
  }

  // 2. Irányítószám formátum ellenőrzése (pontosan 4 számjegy)
  ...
  // Először eltávolítjuk a szóközöket, kötőjeleket és zárójeleket a teszteléshez
  const cleanedPhone = receiver_phone.replace(/[\s\-\(\)]/g, '');
  
  // Magyar telefonszám regex: 
  // Opcionális +36/06/36, utána körzetszám (mobil: 20,30,31,70 vezetékes: 1, 22-99), majd 6-7 számjegy
  ...
  // 4. Minimális címhossz (hogy ne lehessen csak egy pontot vagy szemetet beírni)
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      showAlert("Hiba", error.response?.data?.message || "Hiba történt a mentés során.");
    } finally {
      setLoading(false);
    }
  };
          ...
          <View style={styles.buttonContainer}>
            ...
            >
              {loading ? (
                ...
                <Text style={styles.submitText}>Küldés ({formData.price} Ft)</Text>
              )}
            </TouchableOpacity>
          </View>
        </View>
      ...
      <Modal visible={showConfirmModal} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
             ...
             <Text style={styles.modalText}>Fizetendő: <Text style={{fontWeight:'bold'}}>{formData.price} Ft</Text></Text>
             <View style={styles.modalButtons}>
                ...
                <TouchableOpacity style={styles.modalConfirm} onPress={handleFinalSubmit}>
                  <Text style={{color: '#fff', fontWeight: 'bold'}}>Rendelés</Text>
                </TouchableOpacity>
             </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: "#fff", width: "100%", maxWidth: 500, borderRadius: 20, padding: 20, elevation: 4 },
  sectionTitle: { fontSize: 16, fontWeight: "800", color: "#333", marginBottom: 12 },
  addressBlock: { marginBottom: 10 },
  label: { fontSize: 13, fontWeight: "600", color: "#777", marginBottom: 5, marginTop: 15 },
  ...
  submitBtn: { backgroundColor: "#007AFF", padding: 15, borderRadius: 12, alignItems: "center" },
  submitText: { color: "#fff", fontSize: 16, fontWeight: "bold" },
  modalOverlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.6)", justifyContent: "center", alignItems: "center" },
  modalContent: { backgroundColor: "#fff", borderRadius: 20, padding: 25, alignItems: "center", width: '85%', maxWidth: 400 },
  modalTitle: { fontSize: 20, fontWeight: "bold", marginVertical: 15 },
  modalText: { fontSize: 16, marginBottom: 20 },
  modalButtons: { flexDirection: "row", gap: 10, width: '100%' },
  modalCancel: { flex: 1, padding: 12, alignItems: "center", borderWidth: 1, borderColor: '#ddd', borderRadius: 10 },
  modalConfirm: { flex: 1, padding: 12, alignItems: "center", backgroundColor: '#007AFF', borderRadius: 10 }
});